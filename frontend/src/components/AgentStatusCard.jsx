// AgentStatusCard.jsx — Shows agent lifecycle state and manual trigger
...
export default function AgentStatusCard({ agentStatus, triggering, triggerMessage, onTrigger }) {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 glow-card">
      ...
      {/* Next run */}
      {agentStatus?.nextRunTime && (
        <p className="text-xs text-gray-600 mb-4">
          Next scheduled: {formatTime(new Date(Date.now() - (new Date(agentStatus.nextRunTime) - Date.now())).toISOString())}
        </p>
      )}
      ...
      {triggerMessage && (
        <div className="mb-3 p-2 bg-green-500/10 border border-green-500/20 rounded-lg">
          <p className="text-xs text-green-400">✓ {triggerMessage}</p>
        </div>
      )}
      ...
      <button
        onClick={onTrigger}
        disabled={agentStatus?.isRunning || triggering}
        className="btn-primary w-full py-2.5 px-4 rounded-lg bg-purple-600 hover:bg-purple-500 disabled:bg-gray-700 disabled:text-gray-500 disabled:cursor-not-allowed text-white font-medium text-sm transition-all flex items-center justify-center gap-2"
      >
